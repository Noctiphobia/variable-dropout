...
class TestVariableDropout(TestCase):
    ...
    def test_same_results_with_same_random_state(self):
        ...
        model = LogisticRegression(random_state=rng)
        model.fit(X, y)
        importance_first = variable_dropout_loss(model, X, y, n_iters=1, loss_function=hinge_loss, random_state=0)
        importance_second = variable_dropout_loss(model, X, y, n_iters=1, loss_function=hinge_loss, random_state=0)
    ...
    def test_negative_iters(self):
        with self.assertRaises(ValueError):
            variable_dropout_loss(LinearRegression(), pd.DataFrame({'a': [1, 2, 3], 'b': [4, 5, 6]}), [0, 1, 2],
                                  n_iters=-1)
